...
__inline__ LIBDASM_BYTE FETCH8(LIBDASM_BYTE *addr) {
	// So far byte cast seems to work on all tested platforms
	return *(LIBDASM_BYTE *)addr;	
}

__inline__ LIBDASM_WORD FETCH16(LIBDASM_BYTE *addr) {
#if defined __X86__
	// Direct cast only for x86
	return *(LIBDASM_WORD *)addr;
#else
	// Revert to memcpy
	LIBDASM_WORD val;
	memcpy(&val, addr, 2);
...
__inline__ LIBDASM_DWORD FETCH32(LIBDASM_BYTE *addr) {
#if defined __X86__
	return *(LIBDASM_DWORD *)addr;	
#else
	LIBDASM_DWORD val;
	memcpy(&val, addr, 4);
#if defined __LITTLE_ENDIAN__
...
int get_real_instruction2(LIBDASM_BYTE *addr, int *flags) {
	switch (*addr) {
...
int get_real_instruction(LIBDASM_BYTE *addr, int *index, int *flags) {
	switch (*addr) {
...
int get_operand(PINST inst, int oflags, PINSTRUCTION instruction,
	POPERAND op, LIBDASM_BYTE *data, int offset, enum Mode mode, int iflags) {
	LIBDASM_BYTE *addr = data + offset;
	int index = 0, sib = 0, scale = 0;
	int reg      = LIBDASM_REG_NOP;
	int basereg  = LIBDASM_REG_NOP;
	int indexreg = LIBDASM_REG_NOP;
	int dispbytes = 0;
	enum Mode pmode;
	...
	op->reg      = LIBDASM_REG_NOP;
	op->basereg  = LIBDASM_REG_NOP;
	op->indexreg = LIBDASM_REG_NOP;
	...
	if (inst->modrm) {
		...
		// Register
		reg =  MASK_MODRM_LIBDASM_REG(*addr);

		// Displacement bytes
		// SIB can also specify additional displacement, see below
		if (MASK_MODRM_MOD(*addr) == 0) {
			if ((pmode == MODE_32) && (MASK_MODRM_RM(*addr) == LIBDASM_REG_EBP))
				dispbytes = 4;
			if ((pmode == MODE_16) && (MASK_MODRM_RM(*addr) == LIBDASM_REG_ESI))
				dispbytes = 2;
		...
		// 32-bit mode
		if (pmode == MODE_32) {
			if ((MASK_MODRM_RM(*addr) == LIBDASM_REG_ESP) && 
				...
				if ((basereg == LIBDASM_REG_EBP) && !MASK_MODRM_MOD(*addr)) {
					basereg = LIBDASM_REG_NOP;
						dispbytes = 4;
				}
				if (indexreg == LIBDASM_REG_ESP)
					indexreg = LIBDASM_REG_NOP;
			} else {
				if (!MASK_MODRM_MOD(*addr) && (MASK_MODRM_RM(*addr) == LIBDASM_REG_EBP))
					basereg = LIBDASM_REG_NOP;
				else
		...
		// 16-bit
			...
			switch (MASK_MODRM_RM(*addr)) {
				case 0:
					basereg  = LIBDASM_REG_EBX;
					indexreg = LIBDASM_REG_ESI;
					break;
				case 1:
					basereg  = LIBDASM_REG_EBX;
					indexreg = LIBDASM_REG_EDI;
					break;
				case 2:
					basereg  = LIBDASM_REG_EBP;
					indexreg = LIBDASM_REG_ESI;
					break;
				case 3:
					basereg  = LIBDASM_REG_EBP;
					...
					break;
				case 4:
					basereg  = LIBDASM_REG_ESI;
					indexreg = LIBDASM_REG_NOP;
					break;
				case 5:
					basereg  = LIBDASM_REG_EDI;
					indexreg = LIBDASM_REG_NOP;
					break;
				case 6:
	...
	switch (MASK_AM(oflags)) {
		...
		case AM_LIBDASM_REG:
			op->type = OPERAND_TYPE_LIBDASM_REGISTER;
			op->reg  = MASK_LIBDASM_REG(oflags);
			break;

		// Register indirect encoded in instruction
		case AM_IND:
			op->type    = OPERAND_TYPE_MEMORY;
			op->basereg = MASK_LIBDASM_REG(oflags);
			break;
...
skip_rest:
		...
		case AM_E:
			...
			if ((basereg != LIBDASM_REG_NOP) && (MASK_MODRM_MOD(*addr) == 3)) { 
				op->type = OPERAND_TYPE_LIBDASM_REGISTER;
				op->reg  = basereg;
			}
			break;
		...
		case AM_V:
			op->type = OPERAND_TYPE_LIBDASM_REGISTER;
			op->reg  = MASK_MODRM_LIBDASM_REG(instruction->modrm);
			break;
	}
	return 1;
}
...
int get_operand_string(INSTRUCTION *inst, OPERAND *op,
	enum Format format, LIBDASM_DWORD offset, char *string, int length) {
	...
	if (op->type == OPERAND_TYPE_LIBDASM_REGISTER) {
		// check mode
		...
		switch (MASK_AM(op->flags)) {
			case AM_LIBDASM_REG:
				if (MASK_FLAGS(op->flags) == F_r)
					regtype = LIBDASM_REG_SEGMENT;
				else if (MASK_FLAGS(op->flags) == F_f)
					regtype = LIBDASM_REG_FPU;
				else
					regtype = LIBDASM_REG_GEN_DWORD;
				break;
			case AM_E:
			case AM_G:
			case AM_R:
			...
			// control register encoded in MODRM
			case AM_C:
				regtype = LIBDASM_REG_CONTROL;
				break;
			// debug register encoded in MODRM
			case AM_D:
				regtype = LIBDASM_REG_DEBUG;
				break;
			// Segment register encoded in MODRM
			case AM_S:
				regtype = LIBDASM_REG_SEGMENT;
			...
			// TEST register encoded in MODRM
			case AM_T:
				regtype = LIBDASM_REG_TEST;
				break;
			// MMX register encoded in MODRM
			case AM_P:
			case AM_Q:
				regtype = LIBDASM_REG_MMX;
				break;
			// XMM register encoded in MODRM
			case AM_V:
			case AM_W:
				regtype = LIBDASM_REG_XMM;
				break;
		}
		if (regtype == LIBDASM_REG_GEN_DWORD) {
			 switch (MASK_OT(op->flags)) {
				case OT_b:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", reg_table[LIBDASM_REG_GEN_BYTE][op->reg]);
                                        break;
				case OT_v:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", (mode == MODE_32) ?
						reg_table[LIBDASM_REG_GEN_DWORD][op->reg] :
						reg_table[LIBDASM_REG_GEN_WORD][op->reg]);
                                        break;
				case OT_w:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", reg_table[LIBDASM_REG_GEN_WORD][op->reg]);
					break;
				case OT_d:
					snprintf(string + strlen(string), length - strlen(string),
						"%s", reg_table[LIBDASM_REG_GEN_DWORD][op->reg]);
					break;
			}
		} else
			snprintf(string + strlen(string), length - strlen(string),
		...
		if (MASK_PREFIX_G2(inst->flags))
			snprintf(string + strlen(string),
				...
				reg_table[LIBDASM_REG_SEGMENT][(MASK_PREFIX_G2(inst->flags)) - 1]);
		...
		if (op->basereg != LIBDASM_REG_NOP) {
			snprintf(string + strlen(string), length - strlen(string),
				"%s%s", (format == FORMAT_ATT) ? "%" : "", 
				(mode == MODE_32) ?
				reg_table[LIBDASM_REG_GEN_DWORD][op->basereg] :
				reg_table[LIBDASM_REG_GEN_WORD][op->basereg]);
		}
		// Index register
		if (op->indexreg != LIBDASM_REG_NOP) {
			if (op->basereg != LIBDASM_REG_NOP)
				snprintf(string + strlen(string), length - strlen(string),
					"%s%s", (format == FORMAT_ATT) ? ",%" : "+", 
					(mode == MODE_32) ?
					reg_table[LIBDASM_REG_GEN_DWORD][op->indexreg] :
					reg_table[LIBDASM_REG_GEN_WORD][op->indexreg]); 
			else
				snprintf(string + strlen(string), length - strlen(string),
					"%s%s", (format == FORMAT_ATT) ? "%" : "",
					(mode == MODE_32) ?
		...
		if (inst->dispbytes && (format != FORMAT_ATT)) {
			if ((op->basereg != LIBDASM_REG_NOP) || (op->indexreg != LIBDASM_REG_NOP)) {
 ...
 *
 */
int get_instruction(PINSTRUCTION inst, LIBDASM_BYTE *addr, enum Mode mode) {
	PINST ptr = NULL;
	int index = 0;
	int flags = 0;
	...
	// FPU opcodes
		...
		if (*(addr + index) < 0xc0) {
			...
			ptr = &inst_table4[inst->fpuindex]
				[MASK_MODRM_LIBDASM_REG(inst->opcode)];
		} else {
	...
	if (MASK_EXT(flags) && (MASK_EXT(flags) < EXT_T2)) {
		inst->opcode   = *(addr + index);
		inst->extindex = MASK_MODRM_LIBDASM_REG(*(addr + index + 1));
...
int get_mnemonic_string(INSTRUCTION *inst, enum Format format, char *string, int length) {
	...
	if (MASK_PREFIX_G2(inst->flags) &&
		...
		if (inst->type == INSTRUCTION_TYPE_JMPC)
			snprintf(string + strlen(string), length - strlen(string),
				"%s ", reg_table[LIBDASM_REG_BRANCH][(MASK_PREFIX_G2(inst->flags)) - 1]);
		// Segment override for others
		else
			snprintf(string + strlen(string), length - strlen(string),
				"%s ", reg_table[LIBDASM_REG_SEGMENT][(MASK_PREFIX_G2(inst->flags)) - 1]);
	}
...
int get_operands_string(INSTRUCTION *inst, enum Format format, LIBDASM_DWORD offset,
...
int get_instruction_string(INSTRUCTION *inst, enum Format format, LIBDASM_DWORD offset,
		char *string, int length) {
...
int get_register_type(POPERAND op) {
	...
	switch (MASK_AM(op->flags)) {
		case AM_LIBDASM_REG:
			if (MASK_FLAGS(op->flags) == F_r)
				return LIBDASM_REGISTER_TYPE_SEGMENT;
			else if (MASK_FLAGS(op->flags) == F_f)
				return LIBDASM_REGISTER_TYPE_FPU;
			else
				return LIBDASM_REGISTER_TYPE_GEN;
		case AM_E:
		case AM_G:
		case AM_R:
				return LIBDASM_REGISTER_TYPE_GEN;
		case AM_C:
				return LIBDASM_REGISTER_TYPE_CONTROL;
		case AM_D:
				return LIBDASM_REGISTER_TYPE_DEBUG;
		case AM_S:
				return LIBDASM_REGISTER_TYPE_SEGMENT;
		case AM_T:
				return LIBDASM_REGISTER_TYPE_TEST;
		case AM_P:
		case AM_Q:
				return LIBDASM_REGISTER_TYPE_MMX;
		case AM_V:
		case AM_W:
				return LIBDASM_REGISTER_TYPE_XMM;
		default:
				break;
	}
	return 0;
...
int get_operand_immediate(POPERAND op, LIBDASM_DWORD *imm) {
	if (op->immbytes) {
		*imm = op->immediate;
		return 1;
	} else
...
int get_operand_displacement(POPERAND op, LIBDASM_DWORD *disp) {
	if (op->dispbytes) {
