...
// Data types
typedef unsigned char LIBDASM_BYTE;
typedef unsigned short LIBDASM_WORD;
typedef unsigned long LIBDASM_DWORD;
...
//#endif // other *nix
//#include <sys/types.h>
//typedef u_int8_t  LIBDASM_BYTE;
//typedef u_int16_t LIBDASM_WORD;
//typedef u_int32_t LIBDASM_DWORD;
...
// Registers
#define LIBDASM_REGISTER_EAX 0
#define LIBDASM_REGISTER_ECX 1
#define LIBDASM_REGISTER_EDX 2
#define LIBDASM_REGISTER_EBX 3
#define LIBDASM_REGISTER_ESP 4
#define LIBDASM_REGISTER_EBP 5
#define LIBDASM_REGISTER_ESI 6
#define LIBDASM_REGISTER_EDI 7
#define LIBDASM_REGISTER_NOP 8	// no register defined

// Registers
#define LIBDASM_REG_EAX LIBDASM_REGISTER_EAX
#define LIBDASM_REG_AX LIBDASM_REG_EAX
#define LIBDASM_REG_AL LIBDASM_REG_EAX
#define LIBDASM_REG_ES LIBDASM_REG_EAX          // Just for reg_table consistence
#define LIBDASM_REG_ST0 LIBDASM_REG_EAX         // Just for reg_table consistence
#define LIBDASM_REG_ECX LIBDASM_REGISTER_ECX
#define LIBDASM_REG_CX LIBDASM_REG_ECX
#define LIBDASM_REG_CL LIBDASM_REG_ECX
#define LIBDASM_REG_CS LIBDASM_REG_ECX
#define LIBDASM_REG_ST1 LIBDASM_REG_ECX
#define LIBDASM_REG_EDX LIBDASM_REGISTER_EDX
#define LIBDASM_REG_DX LIBDASM_REG_EDX
#define LIBDASM_REG_DL LIBDASM_REG_EDX
#define LIBDASM_REG_SS LIBDASM_REG_EDX
#define LIBDASM_REG_ST2 LIBDASM_REG_EDX
#define LIBDASM_REG_EBX LIBDASM_REGISTER_EBX
#define LIBDASM_REG_BX LIBDASM_REG_EBX
#define LIBDASM_REG_BL LIBDASM_REG_EBX
#define LIBDASM_REG_DS LIBDASM_REG_EBX
#define LIBDASM_REG_ST3 LIBDASM_REG_EBX
#define LIBDASM_REG_ESP LIBDASM_REGISTER_ESP
#define LIBDASM_REG_SP LIBDASM_REG_ESP
#define LIBDASM_REG_AH LIBDASM_REG_ESP          // Just for reg_table consistence
#define LIBDASM_REG_FS LIBDASM_REG_ESP
#define LIBDASM_REG_ST4 LIBDASM_REG_ESP
#define LIBDASM_REG_EBP LIBDASM_REGISTER_EBP
#define LIBDASM_REG_BP LIBDASM_REG_EBP
#define LIBDASM_REG_CH LIBDASM_REG_EBP
#define LIBDASM_REG_GS LIBDASM_REG_EBP
#define LIBDASM_REG_ST5 LIBDASM_REG_EBP
#define LIBDASM_REG_ESI LIBDASM_REGISTER_ESI
#define LIBDASM_REG_SI LIBDASM_REG_ESI
#define LIBDASM_REG_DH LIBDASM_REG_ESI
#define LIBDASM_REG_ST6 LIBDASM_REG_ESI
#define LIBDASM_REG_EDI LIBDASM_REGISTER_EDI
#define LIBDASM_REG_DI LIBDASM_REG_EDI
#define LIBDASM_REG_BH LIBDASM_REG_EDI
#define LIBDASM_REG_ST7 LIBDASM_REG_EDI
#define LIBDASM_REG_NOP LIBDASM_REGISTER_NOP

// Implied operands
#define IOP_EAX		1
#define IOP_ECX		(1 << LIBDASM_REG_ECX)
#define IOP_EDX		(1 << LIBDASM_REG_EDX)
#define IOP_EBX		(1 << LIBDASM_REG_EBX)
#define IOP_ESP		(1 << LIBDASM_REG_ESP)
#define IOP_EBP		(1 << LIBDASM_REG_EBP)
#define IOP_ESI		(1 << LIBDASM_REG_ESI)
#define IOP_EDI		(1 << LIBDASM_REG_EDI)
#define IOP_ALL		IOP_EAX|IOP_ECX|IOP_EDX|IOP_ESP|IOP_EBP|IOP_ESI|IOP_EDI
#define IS_IOP_LIBDASM_REG(x,y)	(x >> y) & 1
#define IS_IOP_EAX(x)	(x) & 1
#define IS_IOP_ECX(x)	(x >> LIBDASM_REG_ECX) & 1
#define IS_IOP_EDX(x)	(x >> LIBDASM_REG_EDX) & 1
#define IS_IOP_EBX(x)	(x >> LIBDASM_REG_EBX) & 1
#define IS_IOP_EBP(x)	(x >> LIBDASM_REG_EBP) & 1
#define IS_IOP_ESI(x)	(x >> LIBDASM_REG_ESI) & 1
#define IS_IOP_EDI(x)	(x >> LIBDASM_REG_EDI) & 1


// Register types
#define LIBDASM_REGISTER_TYPE_GEN	1
#define LIBDASM_REGISTER_TYPE_SEGMENT   2
#define LIBDASM_REGISTER_TYPE_DEBUG     3
#define LIBDASM_REGISTER_TYPE_CONTROL	4
#define LIBDASM_REGISTER_TYPE_TEST      5
#define LIBDASM_REGISTER_TYPE_XMM       6
#define LIBDASM_REGISTER_TYPE_MMX       7
#define LIBDASM_REGISTER_TYPE_FPU       8
...
// InstructionType types (just the most common ones atm)
...
enum Operand {
	...
	OPERAND_TYPE_LIBDASM_REGISTER,	// register operand (eax, mm0, etc.)
...
typedef struct _INST {
	LIBDASM_DWORD type;		// InstructionType type and flags
...
typedef struct _OPERAND {
	...
	LIBDASM_WORD section;		// Section prefix value
	LIBDASM_DWORD displacement;	// Displacement value
	LIBDASM_DWORD immediate;	// Immediate value
	int flags;		// Operand flags
} OPERAND, *POPERAND;

// struct INSTRUCTION is used to interface the library
typedef struct _INSTRUCTION {
	int length;		// InstructionType length
	enum InstructionType type;	// InstructionType type
	enum Mode mode;		// Addressing mode
	LIBDASM_BYTE opcode;		// Actual opcode
	LIBDASM_BYTE modrm;		// MODRM byte
	LIBDASM_BYTE sib;		// SIB byte
	...
	int flags;		// InstructionType flags
...
int get_instruction_string(
	...
	LIBDASM_DWORD offset,		// instruction absolute address
...
int get_operand_immediate(
	POPERAND op,
	LIBDASM_DWORD *imm		// returned immediate value
);
int get_operand_displacement(
	POPERAND op,
	LIBDASM_DWORD *disp		// returned displacement value
);
...
// InstructionType flags (prefixes)
...
#define AM_LIBDASM_REG 0x00210000	// Register encoded in instruction
...
#define MASK_LIBDASM_REG(x) ((x) & 0x0000000f)



// MODRM byte
#define MASK_MODRM_MOD(x) (((x) & 0xc0) >> 6)
#define MASK_MODRM_LIBDASM_REG(x) (((x) & 0x38) >> 3)
#define MASK_MODRM_RM(x)   ((x) & 0x7)

// SIB byte
#define MASK_SIB_SCALE(x) MASK_MODRM_MOD(x)
#define MASK_SIB_INDEX(x) MASK_MODRM_LIBDASM_REG(x)
